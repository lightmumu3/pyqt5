...
#2.1基础的槽操作
# class Demo1(QWidget):
#     def __init__(self):                #初始化继承
#         super(Demo1,self).__init__()       #自身初始化
#         self.button = QPushButton('start',self)    #实例化button，不要忘记self
#         self.button.clicked.connect(self.change_text)   #点击之后调用函数
#
#     def change_text(self):                 #自定义函数
#         print('change text')
#         self.button.setText('stop')
#         self.button.clicked.disconnect(self.change_text)   #点击之后断开连接
#
...
#         self.button.pressed.connect(self.change_text)   #将时间触发条件由clicked改为 pressed
